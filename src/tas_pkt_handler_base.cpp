...
tas_return_et CTasPktHandlerBase::set_pkt_rsp_ping(tas_pl_cmd_et cmd, tas_client_type_et client_type, const uint32_t* pkt_rsp)
{
	...
	tas_pl1rsp_ping_st* pkt = (tas_pl1rsp_ping_st*)&pkt_rsp[1];

	const uint32_t pl1PktSize = sizeof(tas_pl1rsp_ping_st);
	if (pkt_rsp[0] != 4 + pl1PktSize) {
		...
		if ((mpsRq % 4 != 0) || (mpsRq < TAS_PL2_MAX_PKT_SIZE_MIN) || (mpsRq > TAS_PL2_MAX_PKT_SIZE)) {
			...
			assert(mConInfo.identifier[0] != 0);
			const uint32_t zeros[4] = {};
			if (memcmp(mConInfo.device_id, zeros, 16) == 0) {
...
const uint32_t* CTasPktHandlerBase::get_pkt_rq_ping(tas_pl_cmd_et cmd)
{
	...
	tas_pl1rq_ping_st* pkt = (tas_pl1rq_ping_st*)&mRqBuf[1];
	*pkt = {};
...
{
	tas_pl1rsp_header_st* pkt = (tas_pl1rsp_header_st*)&pkt_rsp[1];

	if ((pkt_rsp[0] == 4 + sizeof(tas_pl1rsp_header_st)) && 
		(pkt->cmd == cmd)) {
