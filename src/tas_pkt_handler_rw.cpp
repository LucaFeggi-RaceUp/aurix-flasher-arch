...
void CTasPktHandlerRw::mPktAdd_SetAddrMapAccModeBaseAddr(uint8_t addr_map, uint16_t acc_mode, uint64_t addr)
{
    ...
    if (addr_map != mPl0AddrMap) {
        tas_pl0rq_addr_map_st* pl0AddrMap = (tas_pl0rq_addr_map_st*)&mRqBuf[mRqBufWi];
        pl0AddrMap->wl = 0;
    ...
    if (acc_mode != mPl0AccMode) {
        tas_pl0rq_acc_mode_st* pl0AccMode = (tas_pl0rq_acc_mode_st*)&mRqBuf[mRqBufWi];
        pl0AccMode->wl = 0;
    ...
    if ((addr < mPl0BaseAddr) || (addr > (mPl0BaseAddr + 0xFFFF))) {
        ...
        if (addr < 0x100000000ll) {
            tas_pl0rq_base_addr32_st* pl0Ba32 = (tas_pl0rq_base_addr32_st*)&mRqBuf[mRqBufWi];
            pl0Ba32->wl = 0;
            ...
            tas_pl0rq_base_addr64_st* pl0Ba64 = (tas_pl0rq_base_addr64_st*)&mRqBuf[mRqBufWi];
            pl0Ba64->wl = 1;
...
void CTasPktHandlerRw::mPktAdd_Rd(uint64_t addr, uint32_t num_bytes, void* data, uint16_t acc_mode, uint8_t addr_map)
{
    ...
    if (num_bytes <= 8) {
        tas_pl0rq_rd_st* pl0Rd = (tas_pl0rq_rd_st*)&mRqBuf[mRqBufWi];
        pl0Rd->wl = 0;
        switch (num_bytes) {
        ...
        tas_pl0rq_rdblk_st* pl0RdBlk = (tas_pl0rq_rdblk_st*)&mRqBuf[mRqBufWi];
        pl0RdBlk->wl = 1;
...
void CTasPktHandlerRw::mPktAdd_Wr(uint64_t addr, uint32_t num_bytes, const void* data, uint16_t acc_mode, uint8_t addr_map)
{
    ...
    if (num_bytes <= 8) {
        tas_pl0rq_wr_st* pl0Wr = (tas_pl0rq_wr_st*)&mRqBuf[mRqBufWi];
        ...
        assert((addr & 0x7) == 0);
        assert((num_bytes & 0x7) == 0);
        tas_pl0rq_wrblk_st* pl0WrBlk = (tas_pl0rq_wrblk_st*)&mRqBuf[mRqBufWi];
...
void CTasPktHandlerRw::mPktAdd_Fill(uint64_t addr, uint32_t num_bytes, uint64_t value, uint16_t acc_mode, uint8_t addr_map)
{
    ...
    tas_pl0rq_fill_st* pl0Fill = (tas_pl0rq_fill_st*)&mRqBuf[mRqBufWi];
...
void CTasPktHandlerRw::mPktFinalize(bool init_next_pl2_pkt)
{
    ...
    tas_pl1rq_pl0_end_st* pl0End = (tas_pl1rq_pl0_end_st*)&mRqBuf[mRqBufWi];
    pl0End->wl = 0;
...
bool CTasPktHandlerRw::mCheckLimits(uint32_t num_bytes_rd, uint32_t num_bytes_wr)
{
...
uint32_t CTasPktHandlerRw::mGetNumBytesAddrMapAccModeBaseAddr(uint8_t addr_map, uint16_t acc_mode, uint64_t addr)
{
    uint32_t nbAddrMap, nbAccMode, nbBaseAddr;
...
bool CTasPktHandlerRw::mCheckAddrMapRulesInPkt(uint8_t addr_map)
{
...
bool CTasPktHandlerRw::mNumTransManageableRd(uint64_t addr, uint32_t num_bytes)
{
    if (num_bytes <= 8) {
        ...
        return ((mPl0NumTrans + numTrans) <= mNumTransMax);
    }
    else {
        ...
        return ((mPl0NumTrans + numTransPayload + numTransAdder) <= mNumTransMax);
    }
}

bool CTasPktHandlerRw::mNumTransManageableWr(uint64_t addr, uint32_t num_bytes)
{
    if (num_bytes <= 8) {
        uint32_t numTrans = 4;  // Worst case for unaligned access
        if (tphrCheckIfNaturalAligned(addr, num_bytes))
...
uint32_t CTasPktHandlerRw::mGetRemainingSizeInPktRq()
{
...
uint32_t CTasPktHandlerRw::mGetRemainingSizeInPktRsp()
{
...
uint32_t CTasPktHandlerRw::mGetWrDataBlkSizeInPktRq(uint32_t num_bytes, uint64_t addr)
{
    assert(num_bytes >= 8);
...
uint32_t CTasPktHandlerRw::mGetRdDataBlkSizeInPktRsp(uint32_t num_bytes)
{
    assert(num_bytes >= 8);
    uint32_t nb = num_bytes;
...
bool CTasPktHandlerRw::rw_set_trans(const tas_rw_trans_st* trans, uint32_t num_trans)
{
    ...
    uint32_t t; bool succ = false;
    for (t = 0; t < num_trans; t++) {
        if (trans[t].type == TAS_RW_TT_RD) {
            ...
            uint64_t value = *(uint64_t*)trans[t].wdata;
...
uint32_t CTasPktHandlerRw::rw_get_rq_size()
{
...
uint32_t CTasPktHandlerRw::rw_get_rsp_size()
{
    assert(mRspSize <= mMaxRspSize + BUF_ALLOWANCE);
    if (mGetPktRqWasCalled == false)
...
uint32_t CTasPktHandlerRw::rw_get_num_pl2_pkt(const uint32_t* rsp, uint32_t num_bytes)
{
    if (mNumPl2Pkt == 0) {
    ...
    uint32_t numBytesDone = 0, numPl2Pkt = 0;
    uint32_t wiPl2Hdr = 0;
    while (true) {
...
tas_return_et CTasPktHandlerRw::rw_set_rsp(const uint32_t* rsp, uint32_t num_bytes)
{
    ...
    uint32_t wi, wiMax, wiPktStartNext, iTrans;
    wiMax = num_bytes / 4;
    ...
    while (wi < wiMax) {
        ...
        if (wi == wiPktStartNext) {
            ...
            tas_pl1rsp_pl0_start_st* pl1Start = (tas_pl1rsp_pl0_start_st*)&rsp[wi+1];
        ...
        tas_pl_cmd_et cmd  = (tas_pl_cmd_et)((rsp[wi] >> 8) & 0xFF);
        
        // TAS_PL1_CMD_PL0_END
        if (cmd == TAS_PL1_CMD_PL0_END) {
            tas_pl1rsp_pl0_end_st* pl1End = (tas_pl1rsp_pl0_end_st*)&rsp[wi];
            if (pl1End->wl != 0) {
                return mSetPktRspErrConnectionProtocol();
            }
        ...
        if (ctprhcPl0CmdIsWrOrFill(cmd)) {
            tas_pl0rsp_wr_st* pkt = (tas_pl0rsp_wr_st*)&rsp[wi];
            if (wl != 0) {
                return mSetPktRspErrConnectionProtocol();
            }
            ...
            tas_pl0rsp_rd_st* pkt = (tas_pl0rsp_rd_st*)&rsp[wi];
            if (wl != pkt->wlrd) {
                return mSetPktRspErrConnectionProtocol();
            }
...
uint32_t CTasPktHandlerRw::rw_get_pl0_trans(const tas_rw_trans_st** pl0_trans, const tas_rw_trans_rsp_st** pl0_trans_rsp)
{
    *pl0_trans     = mPl0Trans;
...
void CTasPktHandlerRw::mSetPktRspErrPl0Data(uint8_t pl_err, tas_return_et tas_err, uint64_t addr, uint8_t addr_map)
{
    ...
    char addrMapStr[32]; addrMapStr[0] = 0;
    if (addr_map > 0)
        snprintf(addrMapStr, 32, "in addr_map %d ", addr_map);

    if (pl_err == TAS_PL0_ERR_DATA) {
        assert((tas_err == TAS_ERR_RW_READ) || (tas_err == TAS_ERR_RW_WRITE));
        mEip->tas_err = tas_err;
        snprintf(mEip->info, TAS_INFO_STR_LEN, "ERROR: %s of addr %lX %sfailed", typeStr, addr, addrMapStr);
    }
        ...
        mEip->tas_err = tas_err;
        snprintf(mEip->info, TAS_INFO_STR_LEN, "ERROR: Set acc_mode not supported for %s of addr %lX %s", typeStr, addr, addrMapStr);
    }
        ...
        mEip->tas_err = tas_err;
        snprintf(mEip->info, TAS_INFO_STR_LEN, "ERROR: %s of addr %lX %swas blocked", typeStr, addr, addrMapStr);
    }
    else {
        mEip->tas_err = tas_err;
        snprintf(mEip->info, TAS_INFO_STR_LEN, "ERROR: %s of addr %lX %sreturned code 0x%2.2X", typeStr, addr, addrMapStr, pl_err);
        assert(false);
    }
}
