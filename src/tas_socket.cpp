...
int CTasSocket::select_socket(const unsigned int msec)
{
	if (mSocketDesc < 0)
		return -1;
...
bool CTasSocket::set_option(int optname, void* arg)
{
	switch(optname)
	{
		case SO_RCVBUF:
...
bool CTasSocket::set_local_port(unsigned short port) 
{
...
unsigned short CTasSocket::get_local_port() 
{
	struct sockaddr_in saddr;
	int saddrLen = sizeof(saddr);
	
	if ((getsockname(mSocketDesc, (sockaddr*)&saddr, (socklen_t*)&saddrLen)) == SOCKET_ERROR) 
	{
...
const char* CTasSocket::get_local_addr() 
{
	...
	if ((getsockname(mSocketDesc, (sockaddr*)&saddr, (socklen_t*)&saddrLen)) == SOCKET_ERROR) 
	{
		...
		return mLocalIpv4; // Unable to get local address
	}
	return inet_ntop(saddr.sin_family, &saddr.sin_addr, mLocalIpv4, INET_ADDRSTRLEN); // Assumes only Ipv4!
}

bool CTasSocket::set_local_addr_and_port(const char* addr, unsigned short port) 
{
	struct sockaddr_in saddr;
