...
const uint32_t* CTasPktHandlerServerCon::get_pkt_rq_server_connect(const char* client_name, const char* user_name, uint32_t client_pid)
{
	...
	tas_pl1rq_server_connect_st* pkt = (tas_pl1rq_server_connect_st*)&mRqBuf[1];
	*pkt = {};
...
tas_return_et CTasPktHandlerServerCon::set_pkt_rsp_server_connect(const uint32_t* pkt_rsp, const tas_server_info_st** server_info, uint64_t* challenge)
{
	...
	*challenge = mServerChallenge;
	...
	tas_pl1rsp_server_connect_st* pkt = (tas_pl1rsp_server_connect_st*)&pkt_rsp[1];
...
const uint32_t* CTasPktHandlerServerCon::get_pkt_rq_get_targets(uint8_t start_index)
{
	...
	mRqBuf[0] = 4 + pl1PktSize;
	tas_pl1rq_get_targets_st* pkt = (tas_pl1rq_get_targets_st*)&mRqBuf[1];
	*pkt = {};
	pkt->wl = (pl1PktSize / 4) - 1;
...
tas_return_et CTasPktHandlerServerCon::set_pkt_rsp_get_targets(const uint32_t* pkt_rsp, uint8_t* num_target, uint8_t* num_now)
{
	*num_target = *num_now = 0;
	...
	tas_pl1rsp_get_targets_st* pkt = (tas_pl1rsp_get_targets_st*)&pkt_rsp[1];
...
void CTasPktHandlerServerCon::get_target_info(const tas_target_info_st** target_info, uint32_t* num_target)
{
	*target_info = (mNumTarget > 0) ? mTargetInfo : nullptr;
	*num_target = mNumTarget;
}
...
tas_return_et CTasPktHandlerServerCon::set_pkt_rsp_get_target_clients(const uint32_t* pkt_rsp, uint8_t* num_client, uint8_t* num_now)
{
	*num_client = *num_now = 0;
	...
	tas_pl1rsp_get_clients_st* pkt = (tas_pl1rsp_get_clients_st*)&pkt_rsp[1];
	...
	if (pkt_rsp[0] != 4 + pl1PktSize + pkt->num_now * sizeof(tas_target_client_info_st)) {
		...
		if (pkt->err == TAS_PL_ERR_NO_ERROR) {
			if (mStartIndex == 0) {
				snprintf(mSessionName, TAS_NAME_LEN16, "%s", pkt->session_name);
			...
			memcpy(mTargetClientInfo, &pkt_rsp[wiTciStart], pkt->num_now * sizeof(tas_target_client_info_st));
			return tas_clear_error_info(mEip);
		}
...
{
	tas_target_client_info_st* tcci0 = (tas_target_client_info_st*)_tcci0;
	tas_target_client_info_st* tcci1 = (tas_target_client_info_st*)_tcci1;
...
{
	qsort(mTargetClientInfo, mNumTargetClient, sizeof(tas_target_client_info_st), ttci_compare_by_connect_time);

	*session_name = mSessionName;
	*session_start_time_us = mSessionStartTimeUs;
	*num_client = mNumTargetClient;
	*target_client_info = mTargetClientInfo;
}
	...
	const char* session_name, const char* session_pw, tas_chl_target_et chl_target, uint64_t chl_param)
{
	...
	mRqBuf[0] = 4 + pl1PktSize;
	tas_pl1rq_session_start_st* pkt = (tas_pl1rq_session_start_st*)&mRqBuf[1];
	*pkt = {};
	pkt->wl = (pl1PktSize / 4) - 1;
...
const uint32_t* CTasPktHandlerServerCon::get_pkt_rq_device_unlock_set_key(tas_dev_unlock_option_et ulo, const void* key, uint16_t key_length)
{
	...
	tas_pl1rq_set_device_key_st* pkt = (tas_pl1rq_set_device_key_st*)&mRqBuf[1];
	*pkt = {};
...
{
	tas_pl1rsp_set_device_key_st* pkt = (tas_pl1rsp_set_device_key_st*)&pkt_rsp[1];
...
const uint32_t* CTasPktHandlerServerCon::get_pkt_rq_device_connect(tas_clnt_dco_et option)
{
	...
	mRqBuf[0] = 4 + pl1PktSize;
	tas_pl1rq_device_connect_st* pkt = (tas_pl1rq_device_connect_st*)&mRqBuf[1];
	*pkt = {};
	pkt->wl = (pl1PktSize / 4) - 1;
...
{
	tas_pl1rsp_device_connect_st* pkt = (tas_pl1rsp_device_connect_st*)&pkt_rsp[1];
...
const uint32_t* CTasPktHandlerServerCon::get_pkt_rq_device_reset_count()
{
	...
	mRqBuf[0] = 4 + pl1PktSize;
	tas_pl1rq_device_reset_count_st* pkt = (tas_pl1rq_device_reset_count_st*)&mRqBuf[1];
	*pkt = {};
	pkt->wl = (pl1PktSize / 4) - 1;
...
{
	tas_pl1rsp_device_reset_count_st* pkt = (tas_pl1rsp_device_reset_count_st*)&pkt_rsp[1];
