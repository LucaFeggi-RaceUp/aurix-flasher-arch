...
CTasClientServerCon::CTasClientServerCon(const char* client_name, tas_error_info_st* ei, CTasPktMailboxIf* mb_if)
	: mTphsc(ei)
{
	snprintf(mClientName, sizeof(mClientName), "%s", client_name);
	...
	tasutil_get_user_name(mUserName);
...
tas_return_et CTasClientServerCon::server_connect(const char* ip_addr, uint16_t port_num)
{
	...
	const uint32_t *pktRq = mTphsc.get_pkt_rq_server_connect(mClientName, mUserName, mClientPid);

	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_server_connect_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	if (mTphsc.set_pkt_rsp_server_connect(pktRsp, &mServerInfo, &mServerChallenge)) {
		return mEip->tas_err;
	}

	assert(strlen(ip_addr) < IP_ADDR_NAME_BUF_SIZE);
	snprintf(mServerIpAddr, IP_ADDR_NAME_BUF_SIZE, "%s", ip_addr);
...
tas_return_et CTasClientServerCon::server_unlock(const void* key, uint16_t key_length)
{
	...
	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_server_unlock_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	if (mTphsc.set_pkt_rsp_server_unlock(pktRsp)) {
		return mEip->tas_err;
	}
...
tas_return_et CTasClientServerCon::get_targets(const tas_target_info_st** target_info, uint32_t* num_target)
{
	*num_target = 0;
	...
	uint32_t pktRsp[TAS_MAX_PKT_SIZE_1KB / 4];
	uint8_t numTarget, numNow, startIndex = 0;

	do {
		const uint32_t* pktRq = mTphsc.get_pkt_rq_get_targets(startIndex);

		if (!mMbIf->execute(pktRq, pktRsp))
			return tas_client_handle_error_server_con(mEip);

		if (mTphsc.set_pkt_rsp_get_targets(pktRsp, &numTarget, &numNow))
			return mEip->tas_err;
...
)
{
	...
	*num_client = 0;
	...
	uint32_t pktRsp[TAS_MAX_PKT_SIZE_1KB / 4];
	uint8_t numClient, numNow, startIndex = 0;

	do {
		const uint32_t *pktRq = mTphsc.get_pkt_rq_get_target_clients(identifier, startIndex);

		if (!mMbIf->execute(pktRq, pktRsp))
			return tas_client_handle_error_server_con(mEip);

		if (mTphsc.set_pkt_rsp_get_target_clients(pktRsp, &numClient, &numNow))
			return mEip->tas_err;
												 ...
												 tas_chl_target_et chl_target, uint64_t chl_param)
{
	...
	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_session_start_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	if (mTphsc.set_pkt_rsp_session_start(client_type, pktRsp)) {
		return mEip->tas_err;
	}
...
tas_return_et CTasClientServerCon::device_unlock_get_challenge(tas_dev_unlock_cr_option_et ulcro, const void** challenge, uint16_t* challenge_length)
{
	...
	*challenge_length = 0;
	...
	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_device_unlock_get_challenge_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	if (mTphsc.set_pkt_rsp_device_unlock_get_challenge(pktRsp, challenge, challenge_length)) {
		return mEip->tas_err;
	}
...
tas_return_et CTasClientServerCon::device_unlock_set_key(tas_dev_unlock_option_et ulo, const void* key, uint16_t key_length)
{
	...
	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_set_device_key_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	if (mTphsc.set_pkt_rsp_device_unlock_set_key(pktRsp)) {
		return mEip->tas_err;
	}
...
tas_return_et CTasClientServerCon::mDeviceConnect(tas_clnt_dco_et dco)
{
	...
	uint32_t pktRsp[(4 + sizeof(tas_pl1rsp_device_connect_st)) / 4];
	if (!mMbIf->execute(pktRq, pktRsp))
		return tas_client_handle_error_server_con(mEip);

	uint16_t devConFeatUsed;
	uint32_t deviceType;
	if (mTphsc.set_pkt_rsp_device_connect(pktRsp, &devConFeatUsed, &deviceType)) {
		return mEip->tas_err;
	}
