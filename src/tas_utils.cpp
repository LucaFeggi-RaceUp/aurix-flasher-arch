...
int tasutil_get_con_info_str(const tas_con_info_st* ci, char* str_buf, bool is_client_chl)
{
	...
	char devConFeat[TASUTIL_DEV_CON_FEAT_STR_BUF_SIZE];
	tasutil_get_dev_con_feat_str(ci->dev_con_feat, devConFeat);

	char ipv4Addr[64];
	if (ci->ipv4_addr != 0) {
		const uint8_t* ipa = (const uint8_t*)&ci->ipv4_addr;
		snprintf(ipv4Addr, 64, "%u.%u.%u.%u (direct TAS Ethernet access supported)", 
			     ipa[3], ipa[2], ipa[1], ipa[0]);
	}
	else {
		snprintf(ipv4Addr, 64, "Information not accessible or not supported");
	}

	char chlSupport[256]; const char* chlHeadline;
	if (is_client_chl) {
		...
		snprintf(chlSupport, 256, "TAS channel support was not checked by this client\n\n\n");
	}
...
int tasutil_get_server_info_str(const char* ip_addr, uint16_t port_num, const tas_server_info_st* si, char* str_buf)
{
	...
	char timeStr[TASUTIL_TIME_STR_BUF_SIZE];
	...
	for (i = 1; i < 32; i++) {
		if (si->supp_protoc_ver & (1 << i)) {
			s += snprintf(protocVer + s, 32 - s, "%d, ", i);
		}
	}
	...
	char* str_buf, int str_buf_size)
{
	...
	for (c = 0; c < (int)num_client; c++) {
		...
		s += snprintf(str_buf + s, str_buf_size - s,
			...
			"    Client sent %lu bytes and received %lu bytes\n",
...
void CTasutilDeviceFrequ::get_timer_frequency(double* frequency_hz, double* max_deviation_hz)
{
	if (mTimestampAfterUs == 0) {
		...
		*max_deviation_hz = 1.0E9;  // 1 GHz
		...
		int frequMaxMHzRoundedDown = (int)(frequMaxMHz * 1.001);   // Compensate if crystal frequency is slightly below
		int frequMinMHzRoundedDown = (int)frequMinMHz;
