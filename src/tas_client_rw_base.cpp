...
{
	mTimeoutMs = TAS_DEFAULT_TIMEOUT_MS;
	mRspBuf = new uint32_t[max_rsp_size / 4];
}
...
tas_return_et CTasClientRwBase::target_ping(tas_con_info_st* con_info)
{
	*con_info = {};
	...
	const uint32_t* pktRq = mTphRw->get_pkt_rq_ping(TAS_PL1_CMD_PING);

	if (!mMbIfRw->execute(pktRq, mRspBuf))
		return tas_client_handle_error_server_con(&mEi);

	if (mTphRw->set_pkt_rsp_ping(TAS_PL1_CMD_PING, TAS_CLIENT_TYPE_RW, mRspBuf)) {
		return mEi.tas_err;
	}
...
tas_return_et CTasClientRwBase::fill32(uint64_t addr, uint32_t value, uint32_t num_bytes, uint8_t addr_map)
{
	...
	tas_return_et ret = execute_trans(trans, t);
...
{
	tas_return_et ret = execute_trans(trans, 1);
	if (ret != TAS_ERR_NONE)
	{
		if (num_bytes_ok)
			*num_bytes_ok = 0;
...
tas_return_et CTasClientRwBase::execute_trans(const tas_rw_trans_st* trans, uint32_t num_trans)
{
	...
	{
		char transStr[TAS_INFO_STR_LEN/2];
		const char* typeStr = (trans->type == TAS_RW_TT_RD) ? "RD" : "WR";
		snprintf(transStr, TAS_INFO_STR_LEN/2, "%s addr=0x%" PRIX64 ", num_bytes=%" PRIu32 ", acc_mode=0x%4.4" PRIX16 ", addr_map=%" PRIu8,
			typeStr, trans->addr, trans->num_bytes, trans->acc_mode, trans->addr_map);
		if (num_trans == 1)
			snprintf(mEi.info, TAS_INFO_STR_LEN, "ERROR: Failed to add %s", transStr);
		else
			snprintf(mEi.info, TAS_INFO_STR_LEN, "ERROR: Failed to add %" PRIu32 " trans (first %s)", num_trans, transStr);
		return TAS_ERR_FN_PARAM;
	}

	const uint32_t* rq;
	uint32_t rqNumBytes, rspNumBytes, numPl2Pkt;
	mTphRw->rw_get_rq(&rq, &rqNumBytes, &rspNumBytes, &numPl2Pkt);

	uint32_t rspNumBytesReceived = 0;
	if (!mMbIfRw->execute(rq, mRspBuf, numPl2Pkt, &rspNumBytesReceived))
		return tas_client_handle_error_server_con(&mEi);
	assert(rspNumBytesReceived > 0);
	assert(rspNumBytesReceived % 4 == 0);
	assert(rspNumBytesReceived <= rspNumBytes);

	if (mTphRw->rw_set_rsp(mRspBuf, rspNumBytesReceived) != TAS_ERR_NONE)
		return mEi.tas_err;
